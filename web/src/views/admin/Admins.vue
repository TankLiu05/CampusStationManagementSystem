...
// 省份列表
const provinces = computed(() => chinaDivisions.map(p => p.name))
...
const availableCities = computed(() => {
  ...
  const province = chinaDivisions.find(p => p.name === adminForm.province)
...
const openCreateModal = () => {
  ...
  if (currentUserRole.value === 'MANAGER' && currentUserProvince.value) {
    ...
    // 尝试在标准省份列表中找到匹配项（处理"河北"与"河北省"的不一致）
    const matched = chinaDivisions.find(p => p.name === userProv || p.name.startsWith(userProv) || userProv.startsWith(p.name))
    
    if (matched) {
      adminForm.province = matched.name
    } else {
